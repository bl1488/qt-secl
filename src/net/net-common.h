...
// Цель
// Проверить навыки кандидата в разработке сетевых приложений с использованием Qt C++, включая работу с сетевыми сокетами (QTcpServer, QTcpSocket), 
// обработку JSON-данных, создание GUI с использованием QMainWindow и QTableWidget, а также эмуляцию данных.

// Задание
// Необходимо разработать клиент-серверное приложение для системы. 
// Сервер представляет собой GUI-приложение, которое управляет подключенными клиентами (устройствами) и отображает их данные в таблицах. Клиент — консольное приложение, 
// эмулирующее устройство, которое передает серверу данные различного типа и длины (метрики сети, статус устройства, логи).
...
// - Отображать в отдельной таблице получаемые от клиентов данные в формате: ID клиента, тип данных (например, "NetworkMetrics" (необходимо распарсить по ключам для отображения в таблицу), 
//   "DeviceStatus"(тоже необходимо распарсить), "Log"), содержимое данных, время получения.
